...
float mediana(int PIN){
  float Aux[N_DATA];
  int i;
  int j;
  float paso,media,cuenta;
  ...
  V_ref[N_DATA - 1] = analogReadMilliVolts(PIN); /* Almacenamos en la última posicion el nuevo valor. */
...
void tension() {
  float med;
  med=mediana(PIN_ADC1);
  V_celula=-0.0000000596427656610408 *med*med+0.004911705419677*med-1.61866365835977;
}

void intensidad() {
  float med;
  med=mediana(PIN_ADC2);
...
void setup() {
  ...
  pinMode(Dac2, OUTPUT);
  tension_celula.attach(0.05,tension);
  ...
  //analogWrite(PIN_PWM,127);-----------------------------------Descomentar--------------------------------
}
